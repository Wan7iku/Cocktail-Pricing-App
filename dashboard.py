...
if df.empty:
    st.error("No data available in the CSV file.")
    st.stop()

# Derived metrics
if "selling_price_after_vat" in df.columns and "cost_after_spillage" in df.columns:
    df["profit"] = (
...
st.title("📊 Cocktail Performance Dashboard")
...
selected_cocktails = st.sidebar.multiselect(
    "Cocktail names",
    options=sorted(df["cocktail_name"].dropna().unique().tolist()),
    default=sorted(df["cocktail_name"].dropna().unique().tolist()),
)
